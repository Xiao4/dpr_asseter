...
function doLog(env) {
	var reqTime = new Date(env.startTime),
		sslText = (env.protocol == 'https'? ' SSL':''),
		newline;
	newline = [env.remoteAddress?env.remoteAddress.replace(/,\s/g, "|"):'0.0.0.0'
			, ' - - [' , reqTime , '] ' 
			, (env.tookTime/1e6) , 'ms' 
			, ' "' , env.method , ' ' , env.url , ' HTTP/' , env.httpVersion , sslText , '" ' 
	...
	fs.stat(logPatch, function(err, stat){
		...
		if(stat.isFile() && stat.size>0){
			var d = new Date(),
				...
				newPath = logPatch.replace(/[^\/]+$/g, '')+'dpr_mixed_log-'+d.getFullYear()+towDigi(d.getMonth()+1)+towDigi(d.getDate())+towDigi(d.getHours())+towDigi(d.getMinutes())+towDigi(d.getSeconds())+'.log';
