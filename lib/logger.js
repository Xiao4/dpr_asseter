...
function cutLogFile () {
	fs.stat(logPatch, function(err, stat){
		...
		if(stat.isFile()){
			var d = new Date(),
				// newPath = logPatch.replace(/[^\/]+$/g, '')+'drp_mixed_log-'+d.valueOf()+'.log';
				newPath = logPatch.replace(/[^\/]+$/g, '')+'drp_mixed_log-'+d.getFullYear()+towDigi(d.getMonth()+1)+towDigi(d.getDate())+towDigi(d.getHours())+towDigi(d.getMinutes())+'.log';
